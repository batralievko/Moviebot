def refs(year,genre):
    ...
    start = 'https://www.imdb.com/search/keyword/?pf_rd_m=A2FGELUUNOQJNL&pf_rd_p=bdc91cb7-0144-4906-b072-b45760c8aa67&pf_rd_r=PXCPH97MT3PHQD6DF4AY&pf_rd_s=right-1&pf_rd_t=15051&pf_rd_i=genre&ref_=kw_ref_gnr&mode=detail&title_type=movie&release_date='
    year_to_year = str(year)+'%2C'+str(year)
    mid = '&page=1&sort=user_rating,desc&genres='
    genre_ = str(genre).title()
    end = '&user_rating=7.5%2C&num_votes=1000%2C'
    url = start + year_to_year + mid + genre_ + end
    driver = webdriver.Chrome()
    links = []
    for i in [random.randint(1,20)]:
    ...
    try:
        ...
        ref_for_im = poster.find('a').get('href')
    except AttributeError:
        poster = 'ref for mem'
    except IndexError:
        poster = 'ref for mem'
